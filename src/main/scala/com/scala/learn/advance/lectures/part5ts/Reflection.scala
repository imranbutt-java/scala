...
object Reflection extends App {
  // Java also provide reflection but in scala we shouldn't use that bcz, scala has vast variety of data types, like
  // Higher Kinded Types etc. So we should better use Scala Reflection
  ...
  // 1 - MIRROR, ClassLoader: is a JVM class that may load other classes during runtime
  val m = ru.runtimeMirror(getClass.getClassLoader)
  // 2 - create a class object = "description"
  // see the clazz type, it would be ClassSymbol and scala made all the types that could be generated from reflection
  // named them as Symbol
